...
function goodVsEvil(good, evil) {
  const forcesOfGoodWorth = [1, 2, 3, 3, 4, 10];
  const forcesOfEvilWorth = [1, 2, 2, 2, 3, 5, 10];
  const [forcesOfGoodScore, forcesOfEvilScore] = [good, evil].map((str, i) =>
    str
      .split(" ")
      .reduce(
        (total, cur, idx) =>
          total + cur * [forcesOfGoodWorth, forcesOfEvilWorth][i][idx],
        0
      )
  );

  if (forcesOfGoodScore > forcesOfEvilScore) {
