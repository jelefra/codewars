...
const findEvenIndex2 = arr => {
  let left = 0;
  let right = arr.reduce((previous, current) => previous + current, 0);
  for (let i = 0; i < arr.length; i++) {
    if (i > 0) left += arr[i - 1];
    right -= arr[i];
...
const sum4 = (arr, from, to) => arr.slice(from, to).reduce((a, b) => a + b, 0);
const findEvenIndex4 = arr =>
  arr.findIndex((el, i) => sum4(arr, 0, i) === sum4(arr, i + 1));

// Practicing
const findEvenIndexZ = arr => {
  return 1;
};
